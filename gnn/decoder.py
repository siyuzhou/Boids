...
def mlp_decoder(features, params, pred_steps, training=False):
    ...
    with tf.variable_scope('decoder_one_step') as scope:
        ...
        num_starting_points = (time_steps + pred_steps - 1) // pred_steps
    ...
    def decoder_one_step(pred_time_series):
        ...
        with tf.variable_scope(scope) as inner_scope:
            with tf.name_scope(inner_scope.original_name_scope):
                ...
                pred_displacement = tf.layers.dense(msg_node_encoded, ndims, name='linear')

                new_step = prev_step + pred_displacement

                pred_time_series = tf.concat([pred_time_series, tf.expand_dims(new_step, 2)],
                                             axis=2)
    ...
    # Reshape pred_time_series, ignore the 0th pred step, which is the starting point.
